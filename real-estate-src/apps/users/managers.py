...
class CustomUserManager(BaseUserManager):
    ...
    def create_superuser(
        ...
        if extra_fields.get("is_staff") is not True:
            raise ValueError(_("superusers must is_staff=True"))

        if extra_fields.get("is_superuser") is not True:
            raise ValueError(_("superusers must is_superuser=True"))

        if extra_fields.get("is_active") is not True:
            raise ValueError(_("superusers must is_active=True"))

        if not password:
            raise ValueError(_("superusers must provide a password"))
